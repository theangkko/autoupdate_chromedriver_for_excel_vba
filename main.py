# original source is  "chromedriver_autoinstaller.utils"
# for vba user, modify some lines
...
def get_similar_chromedriver_version(chrome_version, no_ssl=False):
    """
    Try to find a similar version of ChromeDriver if not found exact Chrome driver version
    ...
    if chrome_version >= "115":
        ...
        minor_version = chrome_version.split(".")[3]
        versions_list = [v['version']for v in good_version_list['versions'] if major_version == v['version'].split(".")[0] ]
        
   
        # diff_version = chrome_version
        most_similar = None
        for ver in versions_list:
            if int(minor_version) > abs(int(minor_version) - int(ver.split(".")[3])):
                most_similar = ver
        return most_similar
...
def download_chromedriver(path: Optional[AnyStr] = None, no_ssl: bool = False):
    ...
    if not chromedriver_version:
        ...
        # return
        chromedriver_version = get_similar_chromedriver_version(chrome_version, no_ssl)
    ...
    if path:
        ...
        chromedriver_dir = os.path.join(os.path.abspath(path))
    else:
...
if __name__ == "__main__":
    ...
    dst_dir = os.path.join(os.environ['LOCALAPPDATA'], 'seleniumbasic')
    
    # Rename the existing chromedriver.exe file if it exists
    chrome_driver = os.path.join(dst_dir, 'chromedriver.exe')
    if os.path.exists(chrome_driver):
        i = 1
        while True:
            new_name = os.path.join(dst_dir, f'chromedriver_({i}).exe')
            if not os.path.exists(new_name):
                os.rename(chrome_driver, new_name)
                break
            i += 1
            
    download_chromedriver(path=dst_dir, no_ssl=False)
